...
// Input varying attributes (from vertex shader)
...
// Constants (explicit type specified)
const vec4 lightColor = vec4(1.0, 1.0, 1.0, 1.0);
const vec4 ambient = vec4(0.1, 0.1, 0.1, 1.0);

// Fog Parameters
#define FOG_STEPS 16
#define FOG_HEIGHT_FALLOFF 0.5

// Exponential decay in y
float getFogDensity(vec3 pos) {
    return exp(-pos.y * FOG_HEIGHT_FALLOFF);
}
...
void main()
{
    ...
    if (fogEnabled == 1) {
        ...
        foggedColor = vec4(surfaceColor, 1.0);
    }

    gl_FragColor = pow(foggedColor, vec4(1.0 / 2.2));
}
