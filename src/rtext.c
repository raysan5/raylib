...
static FontSizeMetric fontSizeMetric = FONT_SIZE_METRIC_ASCENT;
...
void SetFontSizeMetric(FontSizeMetric metric) {
    if(metric != FONT_SIZE_METRIC_ASCENT && metric != FONT_SIZE_METRIC_EM_BOX) {
        TRACELOG(LOG_WARNING, "TEXT: Attempting to set font size metric to invalid enum [%d]");
	return;
    }
