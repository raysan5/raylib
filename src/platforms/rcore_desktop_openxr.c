/**********************************************************************************************
*
*   rcore_desktop_openxr - Functions to manage window, graphics device and inputs
*
...
*       - OpenXR: Interfaces with VR Hardware
*
*
*   LICENSE: zlib/libpng
*
...
*/
    ...
    // now its my turn to fuck things up
    // openxr attempt

    XrResult result = XR_SUCCESS;

    platform.view_type = XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO;
    ...
    if (XR_FAILED(result)) {
        TRACELOG(LOG_FATAL, "Failed to enumerate number of extension properties\n");
        return -1;
    }
    ...
    if (XR_FAILED(result)) {
        TRACELOG(LOG_FATAL, "Failed to enumerate extension properties\n");
        return -1;
    }
    ...
    if(!opengl_supported) {
        TRACELOG(LOG_FATAL, "Runtime does not support OpenGL extension!\n");
        return -1;
    }

    int enabled_ext_count = 1;
    const char* enabled_exts = { XR_KHR_OPENGL_ENABLE_EXTENSION_NAME };
    ...
    strcpy_s(instance_create_info.applicationInfo.applicationName, "Raylib x OpenXR Application");
    strcpy_s(instance_create_info.applicationInfo.engineName, "Raylib");

    result = xrCreateInstance(&instance_create_info, &(platform.instance));
    if (XR_FAILED(result)) {
        TRACELOG(LOG_FATAL, "Failed to create XR instance.\n");
        return -1;
    }

    result = xrGetInstanceProcAddr(platform.instance, "xrGetOpenGLGraphicsRequirementsKHR", (PFN_xrVoidFunction*)&(platform.pfnGetOpenGLGraphicsRequirementsKHR));
    if (XR_FAILED(result)) {
        TRACELOG(LOG_FATAL, "Failed to get OpenGL graphics requirements function!\n");
        return -1;
    }

    XrSystemGetInfo system_get_info = { .type = XR_TYPE_SYSTEM_GET_INFO, .next = NULL, .formFactor = platform.form_factor };
    result = xrGetSystem(platform.instance, &system_get_info, &(platform.system_id));
    if (XR_FAILED(result)) {
        TRACELOG(LOG_FATAL, "Failed to get system for HMD form factor.\n");
        return -1;
    }
    ...
    printf("Successfully got XrSystem with id %llu for HMD form factor\n", platform.system_id);
    {
        ...
        result = xrGetSystemProperties(platform.instance, platform.system_id, &system_props);
        if (XR_FAILED(result)) {
            TRACELOG(LOG_FATAL, "Failed to get System properties\n");
            return -1;
        }
	}

    uint32_t view_count = 0;
    result = xrEnumerateViewConfigurationViews(platform.instance, platform.system_id, platform.view_type, 0, &view_count, NULL);
    if (XR_FAILED(result)) {
        TRACELOG(LOG_FATAL, "Failed to enumerate view configuration views!\n");
        return -1;
    }

    platform.viewconfig_views = malloc(sizeof(XrViewConfigurationView) * view_count);
    for (uint32_t i = 0; i < view_count; i++) {
        viewconfig_views[i].type = XR_TYPE_VIEW_CONFIGURATION_VIEW;
        viewconfig_views[i].next = NULL;
    }

    result = xrEnumerateViewConfigurationViews(platform.instance, platform.system_id, platform.view_type, view_count, &view_count, platform.viewconfig_views);
    if (XR_FAILED(result)) {
        TRACELOG(LOG_FATAL, "Failed to enumerate view configuration views!\n");
        return -1;
    }
    ...
    result = platform.pfnGetOpenGLGraphicsRequirementsKHR(platform.instance, platform.system_id, &opengl_reqs);
    if (XR_FAILED(result)) {
