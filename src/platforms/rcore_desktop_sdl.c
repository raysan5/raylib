            ...
            case SDL_JOYDEVICEADDED:
            {
                ...
                if (!CORE.Input.Gamepad.ready[jid] && (jid < MAX_GAMEPADS))
                {
                    ...
                    if (platform.gamepad[jid])
                    {
                        ...
                        strncpy(CORE.Input.Gamepad.name[jid], SDL_GameControllerNameForIndex(jid), sizeof(CORE.Input.Gamepad.name[jid]) - 1);
...
int InitPlatform(void)
{
    ...
    for (int i = 0; (i < SDL_NumJoysticks()) && (i < MAX_GAMEPADS); i++)
    {
        ...
        if (platform.gamepad[i])
        {
            ...
            strncpy(CORE.Input.Gamepad.name[i], SDL_GameControllerNameForIndex(i), sizeof(CORE.Input.Gamepad.name[jid]) - 1);
            CORE.Input.Gamepad.name[i][sizeof(CORE.Input.Gamepad.name[jid]) - 1] = '\0';
        }
